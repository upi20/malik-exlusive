...
class Refund extends Render_Controller {

	public function index(){
		$peng_id 		= $this->uri->segment(4);
		$this->data['peng_id'] = null;
		$this->title 	= 'Pengadaan';
		$this->content = 'pengadaan-data-refund'; 			
	...
	public function ajax_data_detail()
	{
		...
		// 	$length = 10;
		// }
		$data 	= $this->pengadaan->getAllProdukRefund($length, $start, $cari['value'], $peng_id)->result_array();
		$count 	= $this->pengadaan->getAllProdukRefund(null,null, $cari['value'], $peng_id)->num_rows();
		array($cari);
	...
	public function ubahStatus(){
		...
		$insert['ppru_pend_id'] = $id;
		$insert['ppru_prod_id'] = $get_dt['pend_prod_id'];
		$insert['ppru_jumlah'] 	= $jumlah;
		// $insert['ppru_']
		$this->db->insert('pengadaan_produk_rusak', $insert);
